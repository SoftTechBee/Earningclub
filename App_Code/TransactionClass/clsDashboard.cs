...
public class clsDashboard
{
    ...
    public string Deposit(string username )
    {
        string total = "0";
        try
        {
            string sql = "select sum(credit) as amount from [tblwallet] where username='" + username + "'  ";
            DataTable dt = objcon.ReturnDataTableSql(sql);
            if (dt.Rows.Count > 0)
            {
                total = dt.Rows[0][0].ToString();
  ...
  public string DepositDebit(string username,string type)
    ...
    public string IdActiveDays(string username,string rid)
    {
        ...
        try
        {
            ...
            clsConnection objcon1 = new clsConnection();
            string sql = "SELECT [dbo].[IdActiveDay]('"+username + "','"+ rid + "')";
